...
import hqm.util.Tree;
import hqm.util.TreeImpl;
...
public class HtmlFlusher implements Flusher
{
    ...
    private static final MessageFormat packageTemplate = new MessageFormat("{0}"+SEP+"{1}"+SEP+"{2}"+SEP+"{3}");
    ...
    private Map<URL, List<ClassLoadingInfo>> sortedLocations =
            new TreeMap<URL, List<ClassLoadingInfo>>(new Comparator<URL>() {
    ...
    private void outputAliasTable(PrintWriter pw)
    {
        ...
        {
            pw.println(this.enableHyperlink? 
                "<tr><td><a name=\"a"+entry.getValue()+"\">"+entry.getValue()+"</td><td>"+entry.getKey()+"</td></tr>" :
                "<tr><td>"+entry.getValue()+"</td><td>"+entry.getKey()+"</td></tr>");
        }
    ...
    public void flush(String fileName, Map<ClassLoader, List<ClassLoadingInfo>> classLoaders,
            Map<URL, List<ClassLoadingInfo>> locations, List<ClassLoadingInfo> allClasses, Class[] preLoadClasses) throws IOException
    {
        ...
        this.flush(pw, classLoaders, locations, allClasses, preLoadClasses);
    }
    ...
    private void flushLoaderView(PrintWriter pw, Map<ClassLoader, List<ClassLoadingInfo>> classLoaders, Class[] preLoadClasses)
    {
        ...
        for (Class<?> c : preLoadClasses) {
            ...
            pw.println("<li>" + c.getName() + SEP + "Loader:" + toShort(c.getClassLoader()) + SEP + 
        ...
        for (Map.Entry<ClassLoader, List<ClassLoadingInfo>> entry : classLoaders.entrySet()) {
            ...
            pw.println("<li id=li_" + idSuffix + " onclick=\"onItemClick('" + idSuffix + "')\">" + toShort(loader) + SEP
            ...
            if (entry.getValue() != null && !entry.getValue().isEmpty()) {
                ...
                for (ClassLoadingInfo info : entry.getValue()) {
                    pw.println("<li>" + info.getName() + SEP + toShort(info.getLocation()) + SEP + info.getOrder() + "</li>");
                }
                pw.println("</ul>");
            }
        }
    ...
    private void flushJarView(PrintWriter pw, Map<URL, List<ClassLoadingInfo>> locations)
    {
        ...
        for (Map.Entry<URL, List<ClassLoadingInfo>> entry : locations.entrySet()) {
            ...
            if (entry.getValue() != null && !entry.getValue().isEmpty()) {
                pw.println("<ul id=ul_" + idSuffix + " style=\"{display:none}\">");
                for (ClassLoadingInfo info : entry.getValue()) {
                    pw.println("<li>" + info.getName() + SEP + toShort(info.getLoader()) + SEP + info.getOrder() + "</li>");
                }
                pw.println("</ul>");
            }
        }
    ...
    private void flushPackageView0(PrintWriter pw, Collection<ClassLoadingInfo> allClasses, final MessageFormat msgTemplate)
    {
    ...
    private static final Comparator<Tree<ClassLoadingInfo, String>> comparator = new Comparator<Tree<ClassLoadingInfo, String>>() {
    	...
    	for (Tree<ClassLoadingInfo, String> tree : pkgTree.getChildren()) {
			...
			if (tree.isLeaf()) {
				if (tree.getEntity() != null) {
					...
					item = msgTemplate.format(new Object[] { tree.getId(), toShort(tree.getEntity().getLocation()),
							toShort(tree.getEntity().getLoader()), tree.getEntity().getOrder() });
				}
    ...
    public static void main(String[] args) throws MalformedURLException
    {
        ...
        infos.add(new ClassLoadingInfo("org.hqm.common.MD5", ClassLoader.getSystemClassLoader(), new URL(
            "file:///c:/object.jar"), 0));
        infos.add(new ClassLoadingInfo("org.hqm.common.util.StringUtil", ClassLoader.getSystemClassLoader(), new URL(
            "file:///c:/object.jar"), 1));
        infos.add(new ClassLoadingInfo("org.hqm.tool.JvmInspector", ClassLoader.getSystemClassLoader(), new URL(
            "file:///c:/object.jar"), 2));
        infos.add(new ClassLoadingInfo("org.hqm.tool.uml.UmlTool", ClassLoader.getSystemClassLoader(), new URL(
            "file:///c:/object.jar"), 3));
